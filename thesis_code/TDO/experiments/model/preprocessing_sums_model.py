...
def preprocess_before_running_model_real_world_seb_modfication(source_file_, facts_file_, confidence_value_computation_info_dir_,
									dataitem_index_file_, g_, ancestors_, father_dict, ic_values):

	# load fact information
	header = False
	sources_dataItemValues_ = utils_dataset.load_facts_real_world_seb_modification(facts_file_, header, ancestors_,father_dict, ic_values)
	# removing data item for which there are values not contained in ancestors
	...
	if not (len(os.listdir(confidence_value_computation_info_dir_)) == len(D_)):
		...
		for d in sources_dataItemValues_:
			for v in sources_dataItemValues_[d]:
				if v not in ancestors_:
					remove_d.add(d)
		for d in remove_d:
			del sources_dataItemValues_[d]'''
